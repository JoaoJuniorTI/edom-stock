    ...
    try {
      ...
      const rows = items.map(i=>`
        <tr>
          ...
          <td style="color:#888;font-size:12px">${i.brand}</td>
          <td style="text-align:center">${i.volume_ml}ml</td>
          <td style="text-align:right;font-weight:600;color:#1C1C1E">R$\u00a0${i.price.toFixed(2).replace('.',',')}</td>
        </tr>`).join('')

      const freteRow = frete>0 ? `
        <tr style="border-top:1px solid #e8e0d0">
          <td colspan="3" style="color:#666;font-style:italic">Frete${freteTransp?` · ${freteTransp}`:''}</td>
          <td style="text-align:right;font-weight:600">R$\u00a0${frete.toFixed(2).replace('.',',')}</td>
        </tr>` : ''

      const descontoRow = desconto>0 ? `
        <tr style="border-top:1px solid #e8e0d0">
          <td colspan="3" style="color:#2e7d32;font-style:italic">Desconto${descontoTipo==='percent'?` (${descontoRaw}%)`:''}</td>
          <td style="text-align:right;font-weight:600;color:#2e7d32">- R$\u00a0${desconto.toFixed(2).replace('.',',')}</td>
        </tr>` : ''
...
.page{max-width:700px;margin:0 auto;padding:44px 40px}
.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:36px;padding-bottom:24px;border-bottom:1.5px solid #E8D5A3}
.brand h1{font-family:'Playfair Display',serif;font-size:30px;font-weight:300;color:#A8842C;letter-spacing:0.07em;line-height:1}
.brand p{font-size:10px;letter-spacing:0.18em;text-transform:uppercase;color:#C7C7CC;margin-top:5px;font-weight:500}
.meta{text-align:right;font-size:12px;color:#6C6C70;line-height:1.9}
...
.client{background:#FAF7F0;border:1px solid #EDE0C8;border-radius:10px;padding:16px 20px;margin-bottom:28px}
.client-lbl{font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#C7C7CC;margin-bottom:5px}
.client-name{font-family:'Playfair Display',serif;font-size:18px;color:#1C1C1E;font-weight:400}
.client-contact{font-size:12px;color:#6C6C70;margin-top:2px}
.section-lbl{font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#AEAEB2;margin-bottom:12px}
table{width:100%;border-collapse:collapse}
th{text-align:left;padding:8px 12px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:#B8943F;border-bottom:1.5px solid #E8D5A3;background:#FAF7F0}
th:last-child{text-align:right}th:nth-child(3){text-align:center}
td{padding:11px 12px;border-bottom:1px solid #F5EFE4;font-size:13px}
.total-row{display:flex;justify-content:flex-end;align-items:baseline;gap:16px;margin-top:18px;padding-top:16px;border-top:2px solid #B8943F}
.total-label{font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#AEAEB2}
.total-value{font-family:'Playfair Display',serif;font-size:28px;color:#A8842C;font-weight:300}
.note{margin-top:24px;padding:12px 16px;background:#FAF7F0;border-left:3px solid #B8943F;font-size:12.5px;color:#6C6C70;line-height:1.6;border-radius:0 8px 8px 0}
.footer{margin-top:44px;padding-top:18px;border-top:1px solid #F0EBE0;display:flex;justify-content:space-between;align-items:center}
.footer-brand{font-family:'Playfair Display',serif;font-size:14px;color:#B8943F;letter-spacing:0.07em}
.footer-text{font-size:10.5px;color:#C7C7CC;text-align:right;line-height:1.7}
...
<div class="footer">
  ...
  <div class="footer-text">Orçamento válido por 7 dias<br/>Preços sujeitos à disponibilidade de estoque</div>
      ...
      // Gera PDF real usando blob + link de download
      const blob = new Blob([html], {type:'text/html;charset=utf-8'})
      const url  = URL.createObjectURL(blob)

      // Abre em nova aba e aciona o print (que no mobile vira "Salvar como PDF")
  ...
  return (
    <div className="fade-in">
      ...
      {/* Layout: coluna única no mobile, duas colunas no desktop */}
      <div style={{display:'grid',gridTemplateColumns:'1fr',gap:18}} className="orcamento-grid">
        ...
        <div style={{...glass.card, padding:22}}>
          ...
          <div style={{...glass.card, padding:14,marginBottom:14,background:'rgba(0,0,0,0.025)',boxShadow:'none',borderColor:'var(--b2)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}} className="frete-grid">
              <div>
                ...
                <input style={{...glass.input}} placeholder="Ex: Correios…" value={freteTransp} onChange={e=>setFreteTransp(e.target.value)}/>
              </div>
              <div>
          ...
          <div style={{...glass.card, padding:14,marginBottom:14,background:'rgba(0,0,0,0.025)',boxShadow:'none',borderColor:'var(--b2)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'auto 1fr',gap:10,alignItems:'end'}} className="desconto-grid">
              ...
              <div>
                ...
                <input type="number" style={{...glass.input}} placeholder={descontoTipo==='reais'?'0,00':'0'} step={descontoTipo==='reais'?'0.01':'1'} min="0" max={descontoTipo==='percent'?'100':undefined} value={descontoValor} onChange={e=>setDescontoValor(e.target.value)}/>
              </div>
            </div>
            {desconto>0&&(
          ...
          <button className="btn-gold" onClick={generatePDF} disabled={items.length===0||!clientName||generating} style={{width:'100%',padding:'13px 20px',fontSize:14}}>
            <FileText size={15}/> {generating?'Gerando…':'Gerar PDF'}
          </button>
